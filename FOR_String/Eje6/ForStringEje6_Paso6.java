 ...
 * @author professor
 */
public class ForStringEje6_Paso6 {
        ...
        if (frase.length() >= palabraBuscar.length() && frase.length() > 0) {
            ...
            for (int i = 0; i < frase.length() - (palabraBuscar.length()-1); i++) {
                ...
                if (c == palabraBuscar.charAt(0)){
                    ...
                    //si hemos salido del bucle y la variable sonIguales = true, se ha encontrado y 
                    //guardamos la posición de la i;
                    if (sonIguales == true){
                        //GUARDAMOS LA POSICIÓN DE LA I, DONDE TENEMOS QUE empezar a SUBSTITUIR
                         pos = i;
